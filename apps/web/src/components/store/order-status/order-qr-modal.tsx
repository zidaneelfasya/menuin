...
import { Copy, Check, QrCode, Store, Info } from "lucide-react";
    ...
    <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
      ...
      >
        <DialogHeader className="text-center sm:text-center space-y-1">
          <div className="mx-auto w-10 h-10 rounded-2xl bg-blue-50 text-blue-600 flex items-center justify-center mb-1">
            <QrCode className="w-5 h-5 stroke-[2.2]" style={{ color: "var(--outlet-primary, #0E59F9)" }} />
          </div>
        ...
        <div className="flex flex-col items-center justify-center my-1">
          <div className="p-3 sm:p-3.5 bg-white rounded-2xl border border-gray-200/80 shadow-xs flex items-center justify-center relative">
            <QRCodeSVG
              value={orderNumber}
              size={185}
              level="H"
              includeMargin={false}
              className="w-[170px] h-[170px] sm:w-[185px] sm:h-[185px]"
            />
          </div>
        ...
        <div className="bg-gray-50/80 rounded-xl p-3 border border-gray-150 text-xs space-y-1.5">
          ...
          <div className="flex justify-between items-center pt-1.5 border-t border-gray-200/60">
            <span className="text-gray-700 font-medium">Total Tagihan Tunai:</span>
            <span
