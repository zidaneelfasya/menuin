...
import { modifierGroupSchema, modifierSchema } from '@menuin/validation';
import { ModifierGroupDto } from '@menuin/types';

export async function getModifierGroups(): Promise<{ success: boolean, data?: ModifierGroupDto[], error?: string }> {
  try {
...
export async function createModifierGroup(formData: z.infer<typeof modifierGroupSchema>) {
  try {
    const user = await getCurrentUser();
    if (!user || !user.tenantId) return { success: false, error: 'Unauthorized' };
    
    const validated = modifierGroupSchema.parse(formData);
...
export async function updateModifierGroup(id: string, formData: z.infer<typeof modifierGroupSchema>) {
  try {
    const user = await getCurrentUser();
    if (!user || !user.tenantId) return { success: false, error: 'Unauthorized' };
