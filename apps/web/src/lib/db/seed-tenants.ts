...
const TENANTS_DATA: TenantDef[] = [
  // 1. KOPI SENJA UTAMA (Cafe & Specialty Coffee)
  {
    ...
    isPaid: true,
    users: [
      { name: 'Zidane Elfasya', email: 'zidaneelfasya@gmail.com', role: 'OWNER' },
      { name: 'Budi Santoso (Owner)', email: 'owner.kopi@menuin.com', role: 'MANAGER' },
  ...
  // 2. RESTO PADANG SEDERHANA MINANG (Indonesian Cuisine)
  {
    ...
    isPaid: true,
    users: [
      { name: 'H. Syahril Efendi (Owner)', email: 'owner.padang@menuin.com', role: 'OWNER' },
  ...
  // 3. BOLU & BAKERY IBU ANISA (Pastry, Cake & Bakery)
  {
    ...
    isPaid: true,
    users: [
      { name: 'Ibu Anisa Permata (Owner)', email: 'owner.bakery@menuin.com', role: 'OWNER' },
  ...
  // 4. WARUNG SEMBAKO BERKAH JAYA (Retail & FMCG)
  {
    ...
    isPaid: true,
    users: [
      { name: 'Pak Joko Widodo (Owner)', email: 'owner.sembako@menuin.com', role: 'OWNER' },
  ...
  // 5. KEDAI SEBLAK & BOBA VIRAL (Street Food & Fast Casual - Free Trial)
  {
    ...
    users: [
      { name: 'Rina Marlina (Owner)', email: 'owner.seblak@menuin.com', role: 'OWNER' },
...
async function seedTenants() {
  ...
  try {
    ...
    await db.delete(schema.posDevices);
    await db.delete(schema.tenants);

    // 2. Iterasi tiap Tenant
    for (const tenantData of TENANTS_DATA) {
      ...
      for (const u of tenantData.users) {
        ...
        console.log(`   👤 User [${u.role}]: ${u.email} (Password: password123, PIN: 1234)`);
      }

      const cashierUser = tenantUsers.find((u) => u.role === 'CASHIER') || tenantUsers[0];
      
      // Buat POS Device dummy
      const [posDevice] = await db.insert(schema.posDevices).values({
        tenantId,
        name: 'Main Kasir Device',
        deviceIdentifier: `DEV-${tenantId.substring(0, 8)}`,
      }).returning();
      ...
      for (let d = new Date(thirtyDaysAgo); d <= now; d.setDate(d.getDate() + 1)) {
        ...
        for (let t = 0; t < txCount; t++) {
          ...
          transactionsToInsert.push({
            id: txId,
            tenantId: newTenant.id,
            cashierMembershipId: cashierUser.id,
