...
async function runSeed() {
  ...
  try {
    ...
    // Ambil membershipId pertama yang ada di db
    const memberships = await db.select().from(schema.memberships).limit(1);
    const membershipId = memberships.length > 0 ? memberships[0].id : null;

    if (!membershipId) {
      console.log('⚠️ Peringatan: Tidak ada membership ditemukan. Transaksi akan disimpan tanpa membershipId.');
    }
    ...
    for (let d = new Date(thirtyDaysAgo); d <= now; d.setDate(d.getDate() + 1)) {
      ...
      for (let t = 0; t < txCount; t++) {
        ...
        transactionsToInsert.push({
          id: txId,
          tenantId,
          cashierMembershipId: membershipId,
    ...
    for(let i=0; i<transactionItemsToInsert.length; i+=500) {
      await db.insert(schema.transactionItems).values(transactionItemsToInsert.slice(i, i+500));
    }
