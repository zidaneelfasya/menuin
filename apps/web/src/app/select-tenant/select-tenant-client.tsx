...
import { IconPlus, IconArrowRight, IconLogout } from '@tabler/icons-react';
...
interface SelectTenantClientProps {
  account: AccountInfo | null;
  tenants: TenantItem[];
}

export function SelectTenantClient({ account, tenants }: SelectTenantClientProps) {
  ...
  const handleSelectOutlet = (outletKey: string) => {
    // Trigger the liquid loading animation
    navigateWithTransition(`/outlet/${outletKey}/dashboard`);
    // Persist active tenant cookie in the background
    selectTenantAction(outletKey).catch((err) => {
      console.error('Failed to set tenant context cookie:', err);
    });
  };
          ...
          <div className="flex items-center gap-4">
            ...
            <form action={signOutAction}>
              <Button
                ...
                variant="ghost"
                size="sm"
                className="text-xs font-medium text-slate-600 dark:text-slate-400 hover:text-rose-600 hover:bg-rose-50 dark:hover:bg-rose-950/40 gap-1.5 h-8 px-2.5 rounded-lg transition-colors cursor-pointer"
              >
        ...
        <div className="space-y-10">
          ...
          <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3 items-stretch">
            {tenants.map((tenant, idx) => {
              ...
              return (
                <div key={tenant.tenantId} className="h-full">
                  <button
                    type="button"
                    onClick={() => outletKey && handleSelectOutlet(outletKey)}
                  ...
                  >
                    ...
                    <div className="space-y-4 w-full">
                      ...
                      <div className="flex items-start justify-between gap-4">
                        {tenant.storeLogoUrl ? (
                          ...
                          <div
                            className={`w-16 h-16 sm:w-16 sm:h-16 rounded-2xl bg-gradient-to-br ${gradientClass} text-white font-bold text-2xl flex items-center justify-center shadow-sm shrink-0 group-hover:scale-105 transition-transform duration-300`}
                          >
                        ...
                        <span
                          className={`text-xs font-semibold px-3 py-1 rounded-full border ${roleInfo.className}`}
                        >
                          {roleInfo.label}
                        </span>
                      </div>
                    ...
                    <div className="pt-5 mt-5 border-t border-slate-100 dark:border-slate-800/80 flex items-center justify-between text-xs sm:text-sm w-full text-slate-500 dark:text-slate-400 font-medium">
                      ...
                      <IconArrowRight className="w-4 h-4 group-hover:translate-x-1.5 transition-transform text-slate-400 group-hover:text-blue-600 dark:group-hover:text-blue-400" />
                    </div>
                  </button>
                </div>
              );
